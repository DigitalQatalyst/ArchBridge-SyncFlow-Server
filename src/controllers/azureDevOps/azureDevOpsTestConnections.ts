...
export const createConfiguration = async (
  ...
  try {
    const {
      ...
      name,
      setActive,
    }: CreateConfigurationRequest = req.body;
    ...
    try {
      const testClient = AzureDevOpsClient.createWithConfig({
    ...
    const duplicate = await azureDevOpsConfigStorage.findDuplicateConfiguration(
      patToken.trim(),
      name.trim()
    );
    ...
    let savedConfig;
    try {
      savedConfig = await azureDevOpsConfigStorage.saveConfiguration({
        name: name.trim(),
        patToken: patToken.trim(),
        organization: organization.trim(),
    ...
    if (setActive && testPassed) {
      if (!savedConfig.isActive) {
        const activated = await azureDevOpsConfigStorage.setActiveConfiguration(
          savedConfig.id
        );
        if (activated) {
      ...
      await azureDevOpsConfigStorage.updateConfiguration(savedConfig.id, {
        isActive: false,
      });
...
export const testConnectionWithConfig = async (
  ...
  try {
    ...
    let config;
    if (configId) {
      config = await azureDevOpsConfigStorage.getConfigurationById(configId);
      if (!config) {
        res.status(404).json({
          success: false,
      ...
      config = await azureDevOpsConfigStorage.getActiveConfiguration();
      if (!config) {
        res.status(400).json({
          success: false,
          error:
            "No active configuration found. Please provide a configId or set an active configuration.",
        });
        return;
      }
    }
    ...
    const updatedConfig = await azureDevOpsConfigStorage.updateConfiguration(config.id, {
      isTested: true,
      testPassed,
      testError,
    });
