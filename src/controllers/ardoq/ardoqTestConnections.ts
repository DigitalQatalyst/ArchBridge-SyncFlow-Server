...
import { ArdoqClient } from "../../services/ardoqClient";
...
export const createConfiguration = async (
  ...
  try {
    ...
    try {
      const testClient = ArdoqClient.createWithConfig({
    ...
    const duplicate = await configStorage.findDuplicateConfiguration(
      apiToken.trim(),
      name.trim()
    );
    ...
    let savedConfig;
    try {
      savedConfig = await configStorage.saveConfiguration({
        name: name.trim(),
        apiToken: apiToken.trim(),
        apiHost: host,
        orgLabel: orgLabel?.trim(),
    ...
    if (setActive && testPassed) {
      if (!savedConfig.isActive) {
        const activated = await configStorage.setActiveConfiguration(
          savedConfig.id
        );
        if (activated) {
      ...
      await configStorage.updateConfiguration(savedConfig.id, {
        isActive: false,
      });
...
export const testConnectionWithConfig = async (
  ...
  try {
    ...
    let config;
    if (configId) {
      config = await configStorage.getConfigurationById(configId);
      if (!config) {
        res.status(404).json({
          success: false,
      ...
      config = await configStorage.getActiveConfiguration();
      if (!config) {
        res.status(400).json({
          success: false,
          error:
    ...
    const updatedConfig = await configStorage.updateConfiguration(config.id, {
      isTested: true,
      testPassed,
      testError,
    });
