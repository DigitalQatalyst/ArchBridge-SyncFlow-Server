 ...
 */
interface AzureDevOpsConfigurationRow {
  id: string;
  name: string;
  organization: string;
  pat_token: string;
...
class AzureDevOpsConfigStorage {
   ...
   */
  private rowToConfig(row: AzureDevOpsConfigurationRow): AzureDevOpsConfiguration {
    return {
      id: row.id,
      name: row.name,
   ...
   */
  private configToRow(
    config: Partial<AzureDevOpsConfiguration>
  ): Partial<AzureDevOpsConfigurationRow> {
    const row: Partial<AzureDevOpsConfigurationRow> = {};
    if (config.name !== undefined) row.name = config.name;
    if (config.organization !== undefined) row.organization = config.organization;
    if (config.patToken !== undefined) row.pat_token = config.patToken;
  ...
  async findDuplicateConfiguration(
    patToken: string,
    name: string
  ): Promise<AzureDevOpsConfiguration | null> {
  ...
  async saveConfiguration(
    config: Omit<AzureDevOpsConfiguration, "id" | "createdAt" | "updatedAt">
  ): Promise<AzureDevOpsConfiguration> {
    ...
    const row: Omit<AzureDevOpsConfigurationRow, "created_at" | "updated_at"> = {
      id,
      name: config.name,
    ...
    if (error) {
      ...
      if (
        ...
        throw new Error(
          `A configuration with the same patToken and name already exists`
        );
      }
   ...
   */
  async getAllConfigurations(): Promise<AzureDevOpsConfiguration[]> {
    const { data, error } = await supabase
      .from("azure_devops_configurations")
      .select("*")
  ...
  async getConfigurationById(
    id: string
  ): Promise<AzureDevOpsConfiguration | undefined> {
    const { data, error } = await supabase
      .from("azure_devops_configurations")
      .select("*")
      .eq("id", id)
   ...
   */
  async getActiveConfiguration(): Promise<AzureDevOpsConfiguration | undefined> {
    const { data, error } = await supabase
      .from("azure_devops_configurations")
      .select("*")
      .eq("is_active", true)
  ...
  async updateConfiguration(
    id: string,
    updates: Partial<Omit<AzureDevOpsConfiguration, "id" | "createdAt">>
  ): Promise<AzureDevOpsConfiguration | null> {
   ...
   */
  async setActiveConfiguration(id: string): Promise<AzureDevOpsConfiguration | null> {
    return this.updateConfiguration(id, { isActive: true });
  }
}

export const azureDevOpsConfigStorage = new AzureDevOpsConfigStorage();
