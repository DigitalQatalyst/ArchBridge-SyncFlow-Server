...
exports.createProject = exports.listProjects = exports.getProcessTemplates = void 0;
 ...
 */
const getProcessTemplates = async (req, res) => {
    try {
...
exports.getProcessTemplates = getProcessTemplates;
/**
...
const createProject = async (req, res) => {
    try {
        const configId = req.query.configId;
        const { name, description, visibility, capabilities, } = req.body;
        // Validation
        ...
        if (capabilities === undefined) {
            ...
            if (!agileTemplateId) {
                res.status(500).json({
                    success: false,
                    error: "Failed to retrieve Agile process template. Please ensure your organization has the Agile process template available, or specify a process template in the request.",
                });
                return;
            }
            ...
            capabilities.versioncontrol ?? (capabilities.versioncontrol = {
                sourceControlType: "Git",
            });
            projectPayload.capabilities = capabilities;
        }
        // Create project
        const operationReference = await client.post("/_apis/projects?api-version=7.1", projectPayload);
        res.status(202).json({
            success: true,
        ...
        res.status(statusCode).json({
            ...
            details: error.statusCode
                ? `HTTP ${error.statusCode}: ${error.message || "Project creation failed"}`
                : "Project creation error",
        });
    }
};
exports.createProject = createProject;
//# sourceMappingURL=azureDevOpsProjects.js.map