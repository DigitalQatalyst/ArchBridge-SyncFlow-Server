...
exports.azureDevOpsClient = exports.AzureDevOpsClient = void 0;
...
class AzureDevOpsClient {
    ...
    initialize() {
        ...
        if (!this.patToken) {
            throw new Error("PAT token is required. Provide it via configure() method or AZURE_DEVOPS_PAT_TOKEN environment variable");
        }
        if (!this.organization) {
            throw new Error("Organization is required. Provide it via configure() method or AZURE_DEVOPS_ORGANIZATION environment variable");
        }
        ...
        this.client.interceptors.request.use((config) => {
            if (process.env.NODE_ENV === "development") {
                ...
                console.log(`[Azure DevOps API] ${config.method?.toUpperCase()} ${config.url}`, {
        ...
        this.client.interceptors.response.use((response) => {
            ...
            if (isHtml) {
                ...
                console.error("[Azure DevOps API] Received HTML response instead of JSON:", {
                ...
                const htmlError = {
                    ...
                    message: "The Azure DevOps API returned HTML instead of JSON. This usually indicates an authentication issue. Please verify your PAT token is valid and not expired. Check the server logs for more details.",
            ...
            if (isHtml) {
                ...
                const htmlError = {
                    ...
                    message: `The Azure DevOps API returned HTML (likely a login/error page) instead of JSON. Status: ${error.response?.status || "Unknown"}. This usually indicates an authentication failure - please verify your PAT token is valid and not expired. Check the server logs for response preview.`,
     ...
     */
    async createWorkItem(project, workItemType, patchDocument) {
        this.initialize();
        ...
        const path = `${project}/_apis/wit/workitems/${encodeURIComponent(workItemType)}?api-version=7.1`;
     ...
     */
    async getAgileProcessTemplateId() {
        try {
            const processes = await this.getProcesses();
            const agileProcess = processes.find((p) => p.name && p.name.toLowerCase() === "agile");
     ...
     */
    async deleteWorkItems(project, workItemIds, destroy = false) {
        this.initialize();
    ...
    getConfig() {
        return {
            organization: this.organization || process.env.AZURE_DEVOPS_ORGANIZATION || "not set",
...
exports.azureDevOpsClient = new AzureDevOpsClient();
//# sourceMappingURL=azureDevOpsClient.js.map